...
export default async function DocumentsPage() {
  ...
  if (!overview) {
    return <div className="p-6 bg-white rounded-2xl border border-gray-200">Заповніть налаштування ФОП, щоб генерувати документи.</div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto pb-12 space-y-6">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Документи</h1>
        <p className="text-gray-500 text-lg">Декларація, платіжка, акт, рахунок, інвойс.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-3">Чеклист перед ДПС</h2>
          ...
          <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-3">Податкові попередження</h2>
            {overview.taxWarnings.length === 0 ? (
              <p className="text-sm text-green-700 bg-green-50 rounded-xl px-3 py-2">Критичних попереджень немає.</p>
            ) : (
              <ul className="space-y-2">
                {overview.taxWarnings.map((warning) => (
                  <li key={warning} className="text-sm text-amber-700 bg-amber-50 rounded-xl px-3 py-2">{warning}</li>
                ))}
              </ul>
            )}
          </div>
          ...
          {overview.missingFields.length > 0 && (
            <div className="bg-white rounded-[2rem] border border-red-100 shadow-sm p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-3">Не заповнені реквізити</h2>
              <ul className="space-y-2">
                {overview.missingFields.map((field) => (
                  <li key={field} className="text-sm text-red-700 bg-red-50 rounded-xl px-3 py-2">{field}</li>
                ))}
              </ul>
            </div>
          )}
